...
const CURRENT_ENRICH_VERSION = 3 // v3 adds the `goodFor` audience/occasion chip to generated content.
...
const content = (row: { history?: string; facts?: string[]; tips?: string; notice?: string; good_for?: string }) =>
  ({ history: row.history ?? '', facts: row.facts ?? [], tips: row.tips ?? '', notice: row.notice ?? '', goodFor: row.good_for ?? '' })
...
async function generate(placeId: string, det: Record<string, unknown>, userAuth: string, actor: string): Promise<{ status: string; content?: unknown }> {
  ...
  await fetch(`${REST}?place_id=eq.${encodeURIComponent(placeId)}&prompt_version=eq.${CURRENT_ENRICH_VERSION}`, { method: 'PATCH', headers: sh({ Prefer: 'return=minimal' }), body: JSON.stringify({
    ...
    history: parsed.history, facts: parsed.facts, tips: parsed.tips, notice: parsed.notice, good_for: parsed.goodFor, source,
