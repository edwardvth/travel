...
## ▶ NEXT UP: Plan-tab parity — Phase 0 ✅ Tier 1 ✅; **Suggest Day Parity 🔨 IN PROGRESS** → then **Tier 3 → Tier 2**

The **Plan-tab feature-parity** initiative is fully scoped (spec + plans written, approved, pushed). **Phase 0 (AI) and Tier 1 are done.** **Suggest Day Parity is being built right now (this session)** — see its plan for status; once it lands, the **queued planned work is Tier 3 then Tier 2** (below).
...
- **Build order (locked):** Phase 0 ✅ → Tier 1 ✅ → **Suggest Day Parity 🔨 (in progress)** → **Tier 3** → **Tier 2**.
...
**🔨 Suggest Day Parity — IN PROGRESS this session (not a queued item).** Restoring full-day "Suggest a day" density. Plan + spec are the source of truth: `…plans/2026-06-23-voyager-suggest-day-parity.md` / `…specs/2026-06-23-voyager-suggest-day-parity-design.md`. Summary of what it ships: completeness/window-driven `suggestDay` prompt (fill a morning→evening day by duration, stop count is a consequence ~6–11 not a target; meal anchors; walkable same/adjacent-district clustering; variety guard), capture `time`+`duration`+ new additive `Stop.mealAnchor`, shared `trip/duration.ts` (reused by Tier 2c+3d), traveler-context hook, `suggestPlaces` 5→6, maxTokens 1500→4000, **`suggestDay` on Opus 4.7 (`claude-opus-4-7`)** while enrichment/suggestPlaces stay Sonnet 4.6. When done, this entry moves to "Shipped & live" and Tier 3 becomes the next build.
