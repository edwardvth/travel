...
- `routes/` — `Landing`, `Auth`, `Dashboard` (the home / trips list), `NewTripSheet`, `ShareSheet`.
...
- `components/` — `AppShell`, `AccountMenu` + `AccountSettings`, `TripCard`/`TripRow`, `ui/` (Button, Input, Sheet, Segmented, Skeleton, IconButton), `ConfirmDialog`, `ThemeToggle`, `Logo`, `SplashIntro`.
- `hero/` — the landing hero (cinematic video / explorer modes, typewriter).
