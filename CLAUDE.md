...
- `data/` — TanStack Query hooks: `useTrips` (+ `useDeleteTrip`, `useBackfillCoverImage`), `useProfile`, `useAccountSettings` (global AI/units/theme in localStorage), `useLandmarkImage`, `useLandmarkBackfill`, `useStopDescription` (+ `usePrefetchStopDescriptions`, `applyStopDescription`).
  ...
  - Both surfaces feed the cache: **Guide** uses `useStopDescription` (auto-generates on view); **`StopDetail`'s Generate/Re-generate button** routes through it too — first-time Generate = cache-first (`runGenerate`); a **founder** Re-generate force-writes the shared entry (`regeneratePlace(placeId, true)`); others re-pull the library. (Earlier `StopDetail` called `generateStopDetail` directly and never populated the cache — fixed.) `StopDetail.handleGenerate` resolves the Google `placeId` (via `place-details`) **before** the cache lookup, so a first-time generate of a stop that had no `placeId` still caches — don't re-parallelize that, the lookup needs the id first. **Only placeId stops ever appear in `place_cache`** (by-name stops generate locally, by design). The legacy `Trip.html` app **never** calls `enrich-place`, so legacy generations never enter the cache; the cache fills only from Passage.
...
- **Legacy descriptions contain HTML; new (v3) ones are plain text.** The legacy `Trip.html` app (still in use, same Supabase DB) writes `<p>`/`<strong>` into stop descriptions; the Passage v3 path (`enrich-place` / `generateStopDetail`) emits clean plain text (verified: `place_cache` rows with `model='claude-sonnet-4-6'` have 0 tags). **Guide renders both cleanly** (`richtext.renderProse` strips block tags); **StopDetail uses `formatInline`**, which allow-lists only inline `<strong>/<em>` and so **leaks `<p>` as raw text** — known, left unfixed by owner's choice (switching StopDetail's history/tips render to `renderProse` like Guide would fix it). Detect legacy text via `history like '%<%'`.
